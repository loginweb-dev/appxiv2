...
@section('content')
  ...
  <div class="d-block d-sm-none">
          ...
          <div class="column">
            <a href="{{ route('negocio', $item->slug) }}">
              <figure class="card card-product p-2">
                <div class="img-wrap">
                  <img src="{{ $item->logo ? 'storage/'.$item->logo : 'storage/'.setting('negocios.img_default_negocio') }}">
                </div>
                    <h6 class="title text-center mitext mt-2">{{ $item->nombre }}</h6>
                    <a href="#" data-toggle="collapse" data-target="#collapse{{ $item->id }}" aria-expanded="true" class="mititle mt-2">
                      <i class="icon-action fa fa-chevron-down"></i>
                      <h5 class="title">Leer mas </h5>
                    </a>
                  <div class="collapse text-center" id="collapse{{ $item->id }}" style="">
                      {{ $item->descripcion }}
                  </div>
    ...
    @foreach ($tipos as $item)
        ...
        <h4 class="text-center mitext">{{ $item->nombre }}</h4>
            ...
            <div class="item-slide p-1">
              <figure class="card card-product">
                ...
                <figcaption class="info-wrap text-center">
                  <h6 class="title text-truncate"><a href="#">{{ $value->nombre }}</a></h6>
                </figcaption>
                <h6 class="title text-center mitext mt-2">{{ $value->nombre }}</h6>
