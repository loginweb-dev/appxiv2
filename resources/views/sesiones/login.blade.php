
<div class="card">
	<article class="card-body">
		<h4 class="card-title text-center p-1 mitext"><i class="fa-solid fa-user"></i> Iniciar Sesión</h4>
		<hr>
			<div class="form-group">
			<div class="input-group">
				<div class="input-group-prepend">
					<span class="input-group-text"> <i class="fa fa-user"></i> </span>
				</div>
				<input id="nombre" class="form-control" placeholder="Nombre Completo" value="" type="text" style="background-image: url(&quot;data:image/png;base64,&quot;); background-repeat: no-repeat; background-attachment: scroll; background-size: 16px 18px; background-position: 98% 50%; cursor: auto;" autocomplete="off">
			</div>
			</div> 
			<div class="form-group">
			<div class="input-group">
				<div class="input-group-prepend">
					<span class="input-group-text"> <i class="fa fa-whatsapp"></i> </span>
				</div>
				<input id="phone" class="form-control" placeholder="whatsapp" type="number" value="" style="background-image: url(&quot;data:image/png;base64,&quot;); background-repeat: no-repeat; background-attachment: scroll; background-size: 16px 18px; background-position: 98% 50%; cursor: auto;" autocomplete="off">
			</div> 			
			</div> 
			<div class="form-group input-group">
				<div class="input-group-prepend">
					<span class="input-group-text"> <i class="fa fa-map"></i> </span>
				</div>
				<select name="" id="localidad" class="form-control mr-sm-1">
					{{-- <option value="0"> Elige tu localidades </option> --}}
					@foreach ($localidades as $item)
					  <option value="{{ $item->id }}">{{ $item->nombre }}</option>
					@endforeach
				  </select>
			</div>
			<div class="getpin" hidden>				
				<div class="form-group confimar text-center">
						<input id="pin" class="form-control" placeholder="Ingresan tu pin" type="number">
						<button onclick="setpin()" type="button" class="btn miboton mt-2 btn-block"><i class="fa-solid fa-check-double fa-xl"></i> Confirmar</button>
						{{-- <button onclick="mireload()" type="button" class="btn miboton mt-2"><i class="fa-solid fa-arrows-rotate"></i> Volver a Intentar</button> --}}
				</div>
			</div>
			<div class="form-group setpin">
				<p class="text-center">se enviara noficacion a tu whatsapp, para confirmar.</p>
				<button onclick="getpin()" type="button" class="btn miboton btn-block"><i class="fa-brands fa-whatsapp fa-xl"></i> Enviar pin a WhatsApp</button>
			</div>

	</article>
	</div>
</div>
